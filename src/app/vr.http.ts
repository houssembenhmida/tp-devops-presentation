...
export class VrHttp {
  ...
  public invokeWScript(script: string): Observable<any> {
    if (this.isValidSession()) {
      // alert('Found a valid Session : ' + script + this.model.sessionId.getValue());
  ...
  public authenticatedCall(apiURL: string): Observable<any> {
    const sessionId = this.model.sessionId.getValue();
   ...
   // alert(sessionId);
    return this.http.get(apiURL + '&jsessionid=' + sessionId, {headers: headers, observe: 'response'}).map(res => {
      console.log(JSON.stringify(res));
